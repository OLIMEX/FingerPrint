/*
Change the address and password of  ZF20 module
...
void setup()
{
    ...
    */
    ...
    /* Change password */
    Serial.print(F("Chaning password: "));
    uint32_t new_password = 0x12345678;
    if((resp = finger.SetPassword(new_password)) != INSTRUCTION_OK) {
        finger.PrintResponse(resp);
        while(true);
    } else {
        finger.set_password(new_password);
        Serial.println(F("OK"));
    }

    /* Change address */
    Serial.print(F("Changing address: "));
    uint32_t new_address = 0x87654321;
    if((resp = finger.SetAddress(new_address)) != INSTRUCTION_OK) {
        finger.PrintResponse(resp);
        while(true);
    } else {
        finger.set_address(new_address);
        Serial.println(F("OK"));
    }

    /* Verify new address and password */
